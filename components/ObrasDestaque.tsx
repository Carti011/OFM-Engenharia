...
export default function Portfolio() {
  return (
    <section id="obras" className="py-24 md:py-32 bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {obras.map((obra, i) => (
            <article
              key={i}
              className="group bg-[#111111] border border-[#1e1e1e] hover:border-[#2563eb]/30 rounded-sm overflow-hidden transition-colors duration-200 cursor-default"
            >
              {/* Placeholder de imagem */}
              <div className="relative h-44 bg-[#161616] flex flex-col items-center justify-center gap-2 overflow-hidden">
                <div
                ...
                <svg
                  ...
                  stroke="currentColor"
                  strokeWidth="1.5"
                  className="text-[#2a2a2a] relative z-10"
                >
                  <rect x="3" y="3" width="18" height="18" rx="2" />
                  <circle cx="8.5" cy="8.5" r="1.5" />
                  <polyline points="21 15 16 10 5 21" />
                </svg>
                <span className="text-[10px] text-[#2a2a2a] tracking-widest uppercase relative z-10">
                  Foto do Projeto
                </span>

                {/* Etiqueta de tipo */}
                <span className="absolute top-3 right-3 px-2 py-1 bg-[#0a0a0a]/80 text-[#f97316] text-[10px] font-bold tracking-wide uppercase rounded-sm">
                  {obra.tipo}
                </span>
              </div>
              ...
              {/* Conteúdo */}
              <div className="p-5">
                <p className="text-[10px] text-[#4b5563] tracking-widest uppercase mb-1">
                  {obra.local}
                </p>
                <h3
                  className="text-sm font-bold text-white mb-2 leading-snug group-hover:text-[#93bbff] transition-colors duration-200"
                  style={{ fontFamily: "var(--font-outfit)" }}
                >
                  {obra.titulo}
                </h3>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {obra.etiquetas.map((etiqueta) => (
                    <span
                      key={etiqueta}
                      className="px-2 py-0.5 bg-[#1a1a1a] border border-[#252525] text-[#6b7280] text-[10px] rounded-sm"
                    >
                      {etiqueta}
                    </span>
                  ))}
